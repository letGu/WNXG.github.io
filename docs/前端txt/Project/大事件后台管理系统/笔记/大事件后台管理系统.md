...
### 1.3 安装 VSCode 的 Live Server 插件辅助开发
...
### 2.12 在 ajaxPrefilter 中统一拼接请求的根路径
...
1. 创建 `/article/art_cate.html` 页面，并初始化如下的 UI 结构：
...
1. 修改 UI 结构，为 `选择封面` 按钮添加 `id`，并且在按钮后面添加 `文件选择框`：
   ...
   ```html
   <!-- 选择封面按钮 -->
   <button type="button" class="layui-btn layui-btn-danger" id="btnChooseImage">
     选择封面
   </button>
   <!-- 隐藏的文件选择框 -->
   <input
